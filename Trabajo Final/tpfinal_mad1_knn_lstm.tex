...
\begin{center}
    ...
    {\Large\bfseries Trabajo Final --- KNN vs LSTM}
\end{center}
...
El objetivo es que el estudiante replique de forma cr\'itica y documentada la comparaci\'on entre \textbf{KNN (K-Nearest Neighbors)} (modelo cl\'asico) y \textbf{LSTM (Long Short-Term Memory)} (modelo de aprendizaje profundo), analice los resultados obtenidos y los contraste con los reportados en el paper.
...
Elaborar una explicaci\'on te\'orica de cada uno de los dos algoritmos asignados: \textbf{KNN (K-Nearest Neighbors)} y \textbf{LSTM (Long Short-Term Memory)}. La explicaci\'on debe incluir:
\begin{itemize}[leftmargin=2em,itemsep=2pt]
...
\begin{itemize}[leftmargin=2em,itemsep=2pt]
    \item Para \textbf{KNN (K-Nearest Neighbors)}: Hastie, T., Tibshirani, R. \& Friedman, J. --- \textit{The Elements of Statistical Learning} (\url{https://hastie.su.domains/ElemStatLearn/}). Ver Capítulo 13, Sección 13.3 (K-Nearest Neighbors).
    \item Para \textbf{LSTM (Long Short-Term Memory)}: Goodfellow, I., Bengio, Y. \& Courville, A. --- \textit{Deep Learning} (\url{https://www.deeplearningbook.org/}). Ver Capítulo 10, Sección 10.10 (LSTM).
...
\begin{tabular}{lcc}
        \toprule
        \textbf{M\'etrica} & \textbf{KNN (K-Nearest Neighbors)} & \textbf{LSTM (Long Short-Term Memory)} \\
        \midrule
...
Elaborar un an\'alisis cr\'itico de este fen\'omeno \textbf{para el par asignado (KNN vs LSTM)}, respondiendo al menos las siguientes preguntas:
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
    \item ?`Qu\'e resultado obtiene cada modelo (KNN (K-Nearest Neighbors) vs LSTM (Long Short-Term Memory)) en t\'erminos de \textit{accuracy} y F1? ?`Cu\'al supera al otro en este experimento?
...
\begin{enumerate}[leftmargin=2em,itemsep=3pt]
    ...
    \item \textbf{Explicaci\'on te\'orica de los algoritmos.} Una secci\'on por algoritmo (KNN (K-Nearest Neighbors) y LSTM (Long Short-Term Memory)). Incluir intuici\'on, arquitectura (con especial detalle para el modelo de aprendizaje profundo) e hiperpar\'ametros principales. Apoyarse en figuras o diagramas cuando sea posible.
...
\section{Entregables}
\begin{itemize}[leftmargin=2em,itemsep=2pt]
    \item \codigo{tpfinal\_mad1\_knn\_lstm.ipynb} --- Jupyter Notebook completo y reproducible
    \item \codigo{tpfinal\_mad1\_knn\_lstm.tex} --- C\'odigo fuente de la presentaci\'on Beamer
    \item \codigo{tpfinal\_mad1\_knn\_lstm.pdf} --- PDF compilado de la presentaci\'on
    \item \codigo{tpfinal\_mad1\_knn\_lstm\_chat.json} --- Conversaci\'on completa con la herramienta de IA utilizada. (NO SE SUBE AL REPOSITORIO, SINO QUE SE ENVIA POR MAIL)
\end{itemize}
...
\begin{itemize}[leftmargin=2em,itemsep=2pt]
    ...
    \item Nombrar el archivo como \codigo{tpfinal\_mad1\_knn\_lstm\_chat.json} (o \codigo{.txt}).
\end{itemize}
