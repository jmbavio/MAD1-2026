...
\begin{center}
    ...
    {\Large\bfseries Trabajo Final --- Random Forest vs MLP}
\end{center}
...
El objetivo es que el estudiante replique de forma cr\'itica y documentada la comparaci\'on entre \textbf{Random Forest} (modelo cl\'asico) y \textbf{MLP (Multilayer Perceptron)} (modelo de aprendizaje profundo), analice los resultados obtenidos y los contraste con los reportados en el paper.
...
Elaborar una explicaci\'on te\'orica de cada uno de los dos algoritmos asignados: \textbf{Random Forest} y \textbf{MLP (Multilayer Perceptron)}. La explicaci\'on debe incluir:
\begin{itemize}[leftmargin=2em,itemsep=2pt]
...
\begin{itemize}[leftmargin=2em,itemsep=2pt]
    \item Para \textbf{Random Forest}: Hastie, T., Tibshirani, R. \& Friedman, J. --- \textit{The Elements of Statistical Learning} (\url{https://hastie.su.domains/ElemStatLearn/}). Ver Capítulo 15 (Random Forests).
    \item Para \textbf{MLP (Multilayer Perceptron)}: Goodfellow, I., Bengio, Y. \& Courville, A. --- \textit{Deep Learning} (\url{https://www.deeplearningbook.org/}). Ver Capítulo 6 (Redes neuronales profundas / MLP).
...
\begin{tabular}{lcc}
        \toprule
        \textbf{M\'etrica} & \textbf{Random Forest} & \textbf{MLP (Multilayer Perceptron)} \\
        \midrule
        Accuracy & 99,88\% & 97,00\% \\
        F1-score (macro) & 97,46\% & 82,00\% \\
...
Elaborar un an\'alisis cr\'itico de este fen\'omeno \textbf{para el par asignado (Random Forest vs MLP)}, respondiendo al menos las siguientes preguntas:
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
    \item ?`Qu\'e resultado obtiene cada modelo (Random Forest vs MLP (Multilayer Perceptron)) en t\'erminos de \textit{accuracy} y F1? ?`Cu\'al supera al otro en este experimento?
...
El resultado del paper no implica que MLP (Multilayer Perceptron) sea un modelo inferior en general. Los modelos de aprendizaje profundo tienen ventajas estructurales en ciertos tipos de datos y problemas. El objetivo de esta secci\'on es identificar y demostrar un caso donde \textbf{MLP (Multilayer Perceptron)} supere claramente a \textbf{Random Forest}.
...
\begin{enumerate}[leftmargin=2em,itemsep=3pt]
    ...
    \item \textbf{Explicaci\'on te\'orica de los algoritmos.} Una secci\'on por algoritmo (Random Forest y MLP (Multilayer Perceptron)). Incluir intuici\'on, arquitectura (con especial detalle para el modelo de aprendizaje profundo) e hiperpar\'ametros principales. Apoyarse en figuras o diagramas cuando sea posible.
...
\section{Entregables}
\begin{itemize}[leftmargin=2em,itemsep=2pt]
    \item \codigo{tpfinal\_mad1\_randomforest\_mlp.ipynb} --- Jupyter Notebook completo y reproducible
    \item \codigo{tpfinal\_mad1\_randomforest\_mlp.tex} --- C\'odigo fuente de la presentaci\'on Beamer
    \item \codigo{tpfinal\_mad1\_randomforest\_mlp.pdf} --- PDF compilado de la presentaci\'on
    \item \codigo{tpfinal\_mad1\_randomforest\_mlp\_chat.json} --- Conversaci\'on completa con la herramienta de IA utilizada. (NO SE SUBE AL REPOSITORIO, SINO QUE SE ENVIA POR MAIL)
\end{itemize}
...
\begin{itemize}[leftmargin=2em,itemsep=2pt]
    ...
    \item Nombrar el archivo como \codigo{tpfinal\_mad1\_randomforest\_mlp\_chat.json} (o \codigo{.txt}).
\end{itemize}
