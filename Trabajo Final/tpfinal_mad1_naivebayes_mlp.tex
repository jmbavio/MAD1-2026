...
\begin{center}
    ...
    {\Large\bfseries Trabajo Final --- Naive Bayes vs MLP}
\end{center}
...
El objetivo es que el estudiante replique de forma cr\'itica y documentada la comparaci\'on entre \textbf{Naive Bayes} (modelo cl\'asico) y \textbf{MLP (Multilayer Perceptron)} (modelo de aprendizaje profundo), analice los resultados obtenidos y los contraste con los reportados en el paper.
...
Elaborar una explicaci\'on te\'orica de cada uno de los dos algoritmos asignados: \textbf{Naive Bayes} y \textbf{MLP (Multilayer Perceptron)}. La explicaci\'on debe incluir:
\begin{itemize}[leftmargin=2em,itemsep=2pt]
...
\begin{itemize}[leftmargin=2em,itemsep=2pt]
    \item Para \textbf{Naive Bayes}: Hastie, T., Tibshirani, R. \& Friedman, J. --- \textit{The Elements of Statistical Learning} (\url{https://hastie.su.domains/ElemStatLearn/}). Ver Capítulo 6, Sección 6.6 (Naive Bayes).
...
\begin{tabular}{lcc}
        \toprule
        \textbf{M\'etrica} & \textbf{Naive Bayes} & \textbf{MLP (Multilayer Perceptron)} \\
        \midrule
        Accuracy & 64,59\% & 97,00\% \\
        F1-score (macro) & 48,70\% & 82,00\% \\
        F1-score (ponderado) & 71,00\% & 98,00\% \\
        \bottomrule
\end{tabular}
\end{center}
...
Elaborar un an\'alisis cr\'itico de este fen\'omeno \textbf{para el par asignado (Naive Bayes vs MLP)}, respondiendo al menos las siguientes preguntas:
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
    \item ?`Qu\'e resultado obtiene cada modelo (Naive Bayes vs MLP (Multilayer Perceptron)) en t\'erminos de \textit{accuracy} y F1? ?`Cu\'al supera al otro en este experimento?
...
\begin{enumerate}[leftmargin=2em,itemsep=3pt]
    ...
    \item \textbf{Explicaci\'on te\'orica de los algoritmos.} Una secci\'on por algoritmo (Naive Bayes y MLP (Multilayer Perceptron)). Incluir intuici\'on, arquitectura (con especial detalle para el modelo de aprendizaje profundo) e hiperpar\'ametros principales. Apoyarse en figuras o diagramas cuando sea posible.
...
\section{Entregables}
\begin{itemize}[leftmargin=2em,itemsep=2pt]
    \item \codigo{tpfinal\_mad1\_naivebayes\_mlp.ipynb} --- Jupyter Notebook completo y reproducible
    \item \codigo{tpfinal\_mad1\_naivebayes\_mlp.tex} --- C\'odigo fuente de la presentaci\'on Beamer
    \item \codigo{tpfinal\_mad1\_naivebayes\_mlp.pdf} --- PDF compilado de la presentaci\'on
    \item \codigo{tpfinal\_mad1\_naivebayes\_mlp\_chat.json} --- Conversaci\'on completa con la herramienta de IA utilizada
\end{itemize}
...
\begin{itemize}[leftmargin=2em,itemsep=2pt]
    ...
    \item Nombrar el archivo como \codigo{tpfinal\_mad1\_naivebayes\_mlp\_chat.json} (o \codigo{.txt}).
\end{itemize}
